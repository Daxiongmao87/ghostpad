...
impl LlmManager {
    ...
    pub fn is_model_downloaded(&self, model_ref: &str) -> bool {
        HuggingFaceModel::parse(model_ref)
            .ok()
            .and_then(|model| self.downloader.path_exists(&model))
            .is_some()
    }
    ...
    pub fn get_model_path(&self, model_ref: &str) -> Option<PathBuf> {
        HuggingFaceModel::parse(model_ref)
            .ok()
            .and_then(|model| self.downloader.get_path(&model))
    }
    ...
    fn ensure_model_loaded(&self) -> anyhow::Result<()> {
        ...
        if let Some(gpu) = main_gpu {
            log::info!("Loading model on GPU {}: {}", gpu, model_path.display());
        } else {
        ...
        if let Ok(entries) = fs::read_dir("/sys/class/drm") {
            ...
            for entry in entries.flatten() {
                ...
                if name_str.starts_with("card") && !name_str.contains('-') {
                    ...
                    let _device_path = entry.path().join("device/device");
